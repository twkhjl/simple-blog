...
需求是將「整個公開前台」改成使用 `page_example/stitch_infohub/infohub/code.html` 為主版的視覺語言，並參考 `_1`、`_2` 補足列表密度與文章內頁 editorial 結構。後台不在這次改版範圍內，且前後台樣式必須分離，避免互相污染。

使用者已明確確認：

- 主版為 `page_example/stitch_infohub/infohub/code.html`
- `_1`、`_2` 僅作補充參考
- 改版範圍包含整個公開前台
- 後台維持現狀
...
- `infohub/code.html` 提供首頁主視覺方向：暖色系、玻璃感、editorial 卡片、輕量 top nav、明亮背景
- `_2/code.html` 提供長文型文章頁較完整的 editorial 結構，可作文章內頁主要參考
- `_1/code.html` 提供資訊密度較高的列表頁、側欄與卡片節奏，可作局部補充
...
### 3. 以 `infohub` 為主，`_2` 為文章頁補強

- 首頁與整體品牌語言以 `infohub` 為準
- 文章內頁吸收 `_2` 的 hero、作者資訊、長文節奏與 related articles 感受，但只保留已存在功能所需區塊
- `_1` 只在卡片密度、列表節奏、次要資訊排列上提供輔助，不作主視覺來源
...
- 用 `infohub` 的 hero section 當首頁開場
- 將統計/說明區塊改寫成前台 editorial summary panel
- 文章卡片以 `infohub` 卡片語言呈現：
  - 封面圖
  - 日期
  - slug 或次要 metadata
  - title
  - excerpt
  - CTA
- 若現有資料不足以支撐範本中的分類、閱讀時間、作者頭像等欄位，僅呈現現有可用欄位，不造假資料
...
這次需求屬於「公開前台整體換皮」，不是單純 CSS 覆蓋。最佳做法是以前台獨立視覺系統重切版面，保留現有 router、auth、API、i18n 與資料流程，同時把前後台樣式明確分離。`infohub` 作為首頁與整體品牌主版，`_2` 作為文章內頁 editorial 補強，登入/註冊/個人頁則以相同 brand language 延伸設計。
